...
Ext.define('MarkdownPanel.singleton.DomEvents', {singleton:true, alternateClassName:['MdDomEvents'], constructor:function(config) {
  this.initConfig(config);
  return this;
}, target:false, clickCheck:function(item) {
  ...
  if (src !== '') {
    ...
    return {type:'img', src:src, link:href, format:'image', extension:src.split('.').pop(), alt:this.att('alt'), title:this.att('title')};
  } else {
    if (href !== '') {
      var type;
      ...
      return {type:type, href:href, link:href, format:'url', extension:href.split('.').pop(), target:target, innerHTML:this.html()};
    } else {
      return false;
    }
  }
...
Ext.define('MarkdownPanel.singleton.Lightbox', {requires:[], singleton:true, alternateClassName:['MdLightbox'], config:{lightBox:null, lightBoxDefaultOptions:{dimensions:true, captions:false, prevImg:false, nextImg:false, hideCloseBtn:false, closeOnClick:true, loadingAnimation:200, animElCount:1, preload:false, carousel:false, animation:400, nextOnClick:true, responsive:true, maxImgSize:0.8, keyControls:true, onopen:Ext.emptyFn, onclose:Ext.emptyFn, onload:Ext.emptyFn, onresize:Ext.emptyFn, onloaderror:Ext.emptyFn}, 
box:false, wrapper:false, thumbnails:[]}, constructor:function(config) {
  this.initConfig(config);
...
Ext.define('MarkdownPanel.toolbar.BrowseTbar', {extend:'Ext.Toolbar', xtype:'MdBrowseTbar', requires:['Ext.button.Button'], defaults:{xtype:'button', ui:'plain-toolbar', disabled:true}, firstPageText:'First Page', backwardText:'Previous Page', forwardText:'Next Page', lastPageText:'Last Page', initComponent:function() {
  var me = this;
  Ext.apply(this, {items:[{iconCls:'fa fa-fast-backward', tooltip:me.firstPageText, itemId:'firstpage', handler:function(b) {
    b.ownerCt.fireEvent('firstpage');
  }}, {iconCls:'fa fa-backward', tooltip:me.backwardText, itemId:'backward', handler:function(b) {
    b.ownerCt.fireEvent('backward');
  }}, {iconCls:'fa fa-forward', tooltip:me.forwardText, itemId:'forward', disabled:true, handler:function(b) {
    b.ownerCt.fireEvent('forward');
  }}, {iconCls:'fa fa-fast-forward', tooltip:me.lastPageText, itemId:'lastpage', handler:function(b) {
...
Ext.define('MarkdownPanel.panel.MarkdownPanel', {extend:'Ext.panel.Panel', alternateClassName:['MdPanel', 'MarkdownPanel'], xtype:'MdPanel', requires:['Ext.layout.container.Fit', 'MarkdownPanel.singleton.DomEvents', 'MarkdownPanel.singleton.Lightbox', 'MarkdownPanel.toolbar.BrowseTbar'], bodyPadding:'0 20 20 20', autoScroll:true, layout:'fit', options:{}, contentWidth:'regular', browseHistory:[], currentPage:-1, cls:'markdown-body', config:{rootFolder:'', rootBook:null, rootDocument:null, lineNumbers:false, 
remarkableDefaults:{html:true, xhtmlOut:false, breaks:false, langPrefix:'language-', linkify:true, typographer:true, quotes:'“”‘’'}}, remarkableOptions:{}, Markdown:null, failureText:'Config failure, see console log for more information', ajaxErrorText:'Ajax failure with error code: ', markdownErrorText:'Markdown Panel requires a "rootFolder" or "rootBook"', page404:'Page not found', failTitle:'Error', html:'', initComponent:function() {
  var me = this;
  me.browseHistory = [];
  Ext.apply(me, {dockedItems:[{dock:'top', xtype:'MdBrowseTbar', hidden:true, listeners:{scope:me, firstpage:me.firstPage, backward:me.backward, forward:me.forward, lastpage:me.lastPage}}]});
  Ext.apply(me.options, me.getRemarkableDefaults());
  me.Markdown = new Remarkable('full', me.options);
  me.Markdown.core.ruler.enable(['abbr']);
  me.Markdown.renderer.rules.image = function() {
    ...
    return function(tokens, idx) {
      ...
      srcSplit.slice(1).map(function(val, ix) {
        srcSplit[ix + 1] = decodeURI(val).replace('+', ' ');
      });
      var imgOutput, outputSplit, newTag, ix;
      ix = srcSplit.indexOf('bordered');
      if (ix >= 0) {
        srcSplit[ix] = 'class\x3d"bordered"';
      }
  ...
  panel.markdown({rootBook:panel.getRootBook(), rootDocument:panel.getRootDocument()});
  panel.body.on('click', function(item, e) {
    var media = MdDomEvents.clickCheck(item);
    if (media !== false) {
      item.stopEvent();
      if (media.extension.toLowerCase() === 'md') {
        panel.markdown({rootBook:panel.getRootBook(), rootDocument:media.link});
      } else {
        if (media.type === 'www') {
          window.open(media.href, media.target);
        } else {
          if (media.type === 'anchor') {
  ...
  Ext.Ajax.request({url:path, success:function(response) {
    ...
    if (response.status === 404) {
      Ext.Msg.show({title:me.failTitle, message:me.page404, icon:Ext.Msg.ERROR, buttons:Ext.Msg.OK});
    }
    return me.failureText;
  }});
}, privates:{validate:function(args) {
  var me = this;
  var fail = false;
  ...
  output = '\x3cdiv class\x3d"markdown-body-wrapper ' + me.contentWidth + '"\x3e' + output + '\x3c/div\x3e';
  me.update(output);
  me.scrollTo(0, 0);
  me.highlightAll();
...
}, privates:{highlightAll:function() {
  var me = this;
  var dom = me.body.dom;
